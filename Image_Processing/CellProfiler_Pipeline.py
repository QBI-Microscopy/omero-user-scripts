...
def get_user_annotations(conn,ext):
    ...
    filtered_anns = [ann[0] for ann in zip(annotations,annotation_names) if ext in ann[1]]
    filtered_names = [ann[1] for ann in zip(annotations,annotation_names) if ext in ann[1]]
    return filtered_anns,filtered_names
        
def temporary_connection():
    tempclient = omero.client()
    omeroProperties = tempclient.getProperties().getPropertiesForPrefix('omero')
    tempclient.closeSession()
    # Configuration
...
def runScript():
    ...
    """
    temp_conn = temporary_connection()
    anns,names = get_user_annotations(temp_conn,"cppipe")
...
""",
    ...
    scripts.Int("Pipeline", optional=False, grouping="3",values=names,
    ...
    try:
        ...
        for key in client.getInputKeys():
            if client.getInput(key):
                ...
                pipeIdx = names.index(scriptParams['Pipeline'])
